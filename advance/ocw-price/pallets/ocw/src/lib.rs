...
pub mod pallet {
	...
	const HTTP_REMOTE_POLKADOT_API: &str = "https://api.coincap.io/v2/assets/polkadot";
	...
	impl fmt::Debug for CoinCapData {
		// `fmt` converts the vector of bytes inside the struct back to string for
		//   more friendly display.
		fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
			write!(
	...
	impl<T: Config> ValidateUnsigned for Pallet<T> {
		...
		-> TransactionValidity
		{
			...
			match call {
				...
				Call::submit_dot_price_unsigned_with_signed_payload(ref pricepayload, ref signature) => {
					if !SignedPayload::<T>::verify::<T::AuthorityId>(pricepayload, signature.clone()) {
						return InvalidTransaction::BadProof.into();
					}
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_dot_price_unsigned_with_signed_payload(origin: OriginFor<T>, payload: PricePayload<T::Public>,
														  _signature: T::Signature) -> DispatchResult
		{
			let _ = ensure_none(origin)?;
			// we don't need to verify the signature here because it has been verified in
			...
			Self::append_or_replace_dot_price(price);
	...
	impl<T: Config> Pallet<T> {
		...
		fn append_or_replace_dot_price(price: (u64, Permill)) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back(price);
				log::info!("dot price vector: {:?}", prices);
			});
		}
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			// 从互联网上获取dot的http response
			let resp_bytes = Self::fetch_from_remote(HTTP_REMOTE_POLKADOT_API).map_err(|e| {
				log::error!("fetch from {}, got error: {:?}",HTTP_REMOTE_POLKADOT_API,e);
				Error::<T>::HttpFetchingError
			})?;

			// 将 response的 byte 变为 string
			let resp_str = str::from_utf8(&resp_bytes).map_err(|e| {
				log::error!("convert response bytes to string error: {:?}",e);
				Error::<T>::HttpFetchingError
			})?;

			log::info!("response from {} is :{}",HTTP_REMOTE_POLKADOT_API, resp_str);

			// 反序列化为对象
			let coin_cap_data: CoinCap = serde_json::from_str(&resp_str).map_err(|e| {
				log::error!("parse json failed:{:?}",e);
				Error::<T>::HttpFetchingError
			})?;

			//获取感兴趣的dot价格
			let dotPrice =CoinCapData{
				symbol: coin_cap_data.data.symbol,
				name: coin_cap_data.data.name,
				priceUsd: coin_cap_data.data.priceUsd
			};

			//log::info!("Price info of Dot is : {:?}", dotPrice);

			// 将小数字符，拆分成整数部分和小数部分组成的元组
			let price_str = str::from_utf8(&dotPrice.priceUsd)
				.map_err(|e| {Error::<T>::ConvertPriceErr})?;

			let price_array: Vec<&str> = price_str.split(".").collect();
			let integer_price_str: &str = &price_array[0];
			let decimal_price_str: &str =&price_array[1][0..4];

			let integer_price = integer_price_str.parse::<u64>()
				.map_err(|e|{Error::<T>::ConvertPriceErr})?;
			let decimal_price =decimal_price_str.parse::<u64>()
				.map_err(|e| {Error::<T>::ConvertPriceErr})?;

			let price_usd: (u64, Permill) =(integer_price as u64, Permill::from_parts(decimal_price as u32));


			let signer = Signer::<T, T::AuthorityId>::any_account();

			if let Some((_, res)) = signer.send_unsigned_transaction(
				|acct| PricePayload { price:price_usd, public: acct.public.clone() },
