...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[weight = 0] //交易的权重，交易消耗的时间
		pub fn create_claim(origin, claim: Vec<u8>) -> dispatch::DispatchResult{
				let sender = ensure_signed(origin)?;

				ensure!(!Proofs::<T>::contains_key(&claim), Error::<T>::ProfAlreadyExist);
		...
		#[weight = 0] //交易的权重，交易消耗的时间
		pub fn transfer_claim(origin, target: T::AccountId, claim: Vec<u8>) -> dispatch::DispatchResult{
			let sender = ensure_signed(origin)?;
			...
			Self::deposit_event(RawEvent::ClaimTransfered(sender, target, claim));
